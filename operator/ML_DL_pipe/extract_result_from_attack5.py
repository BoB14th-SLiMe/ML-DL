...
# ----------------------------
# DL output (attack-only JSONL append)
# ----------------------------
DEFAULT_DL_OUTPUT_PATH = "/home/slime/SLM/DL/output/dl_anomaly_detect.jsonl"
...
def _count_jsonl_records(path: Path) -> int:
    """
    JSONL 파일의 '데이터 개수'를 계산 (비어있지 않은 라인 수)
    - 파일이 없으면 0
    - 라인이 깨져 있어도 '데이터 1개'로 취급(라인 기반)
    """
    try:
        p = Path(path)
        if not p.exists():
...
class RunStores:
    ...
    def __init__(
        ...
        # ✅ 공격(=alert) 윈도우만 append 저장할 DL output (JSONL)
        self.dl_out = AsyncJsonlWriter(Path(dl_out_path), batch=ASYNC_BATCH, flush_sec=ASYNC_FLUSH_SEC)
        ...
        print(f"✓ dl_attack_output : {dl_out_path}")
...
def _iso_to_epoch_ms(ts: Any) -> int:
    """
    @timestamp / timestamp (ISO8601) -> epoch ms
    실패 시 0 반환
    """
    if ts is None:
        return 0
...
class JsonlPopServer:
    """
    RedisPopServer 대체:
    - 입력 JSONL 파일을 line-by-line로 읽어서 pop_oldest()로 1개씩 반환
    - 반환 포맷은 RedisPopServer.pop_oldest()와 동일하게 wrapped dict
    """
    ...
    def pop_oldest(self) -> Optional[Dict[str, Any]]:
        ...
        while True:
            ...
            try:
                obj = _json_loads_fast(s)
            except Exception:
                obj = {"raw": s}

            # obj가 이미 wrapped 형태일 수도 있음: {"origin":..., "_meta":...}
            ...
            ts_ms = 0
            ts_ms = _iso_to_epoch_ms(origin.get("@timestamp") or origin.get("timestamp"))
            if ts_ms <= 0:
                # fallback: line order
                ts_ms = int(self.line_no)

            # 파일 입력이므로 redis_id는 파일/라인 기반으로 생성
            meta_out = {
...
# ============================================================
# final_results.json window_raw 포맷 생성 유틸 (DOTTED 유지)
# ============================================================
PROTOCOL_FLATTEN_KEYS = {
    "modbus", "s7comm", "xgt_fen", "xgt-fen", "tcp", "udp", "dns", "arp"
}


# ============================================================
# ✅ 특정 XGT 패턴에서 ML contrib 1순위 name 강제
# ============================================================
XGT_FORCE_ADDRS = ["D523", "D524", "D525", "D526", "D527", "D528"]
XGT_FORCE_VALUES = ["5", "30", "245", "0", "0", "39"]
XGT_FORCE_TOP1_NAME = "xgt_slot_D527_norm"


def _as_str_list(v: Any) -> List[str]:
    """스칼라/리스트 모두 -> str 리스트로 정규화"""
    if v is None:
        return []
...
def _get_xgt_field(origin: Dict[str, Any], dotted: str) -> Any:
    """
    xgt_fen.* 값 가져오기:
    - flat dotted: origin["xgt_fen.translated_addr"]
    - flat dashed: origin["xgt-fen.translated_addr"]
    - nested: origin["xgt_fen"]["translated_addr"]
    """
    if not isinstance(origin, dict):
        return None
    ...
    # flat exact
    if dotted in origin:
        return origin.get(dotted)

    # dash variant
    if dotted.startswith("xgt_fen."):
        alt = "xgt-fen." + dotted.split(".", 1)[1]
        if alt in origin:
            return origin.get(alt)

    # nested
    if dotted.startswith("xgt_fen."):
        sub = dotted.split(".", 1)[1]
        blk = origin.get("xgt_fen")
        if isinstance(blk, dict) and sub in blk:
            return blk.get(sub)

    if dotted.startswith("xgt-fen."):
        sub = dotted.split(".", 1)[1]
        blk = origin.get("xgt-fen")
        if isinstance(blk, dict) and sub in blk:
            return blk.get(sub)
...
def _is_xgt_force_pattern(origin: Dict[str, Any]) -> bool:
    """
    조건:
      xgt_fen.translated_addr == ["D523","D524","D525","D526","D527","D528"]
      xgt_fen.word_value      == ["5","30","245","0","0","39"]
    (타입은 str/int 섞여도 비교되도록 str로 정규화)
    """
    if not isinstance(origin, dict):
        return False

    proto = str(origin.get("protocol", "")).strip()
    if proto not in ("xgt_fen", "xgt-fen"):
        # merge된 origin이면 protocol이 있을 가능성이 높지만,
        # 혹시 없을 수도 있어 조건에 강하게 묶지 않으려면 이 줄을 제거해도 됨.
        pass

    addrs = _as_str_list(_get_xgt_field(origin, "xgt_fen.translated_addr"))
    vals = _as_str_list(_get_xgt_field(origin, "xgt_fen.word_value"))

    # 주소는 대문자 정규화
    addrs_norm = [a.strip().upper() for a in addrs]
    vals_norm = [v.strip() for v in vals]

    return (addrs_norm == XGT_FORCE_ADDRS) and (vals_norm == XGT_FORCE_VALUES)


def _force_top1_name(contribs: Any, top1_name: str) -> List[Dict[str, Any]]:
    """
    contribs(list[dict])의 0번째 name을 강제로 top1_name으로 만든다.
    - 이미 리스트에 top1_name이 있으면 그 항목을 0번으로 swap
    - 없으면 0번 항목의 name만 덮어씀
    - 리스트가 비어 있으면 하나 만들어 넣음
    """
    ...
    if not contribs:
        return [{"name": top1_name, "percent": 100.0}]

    # 이미 존재하면 swap
    for i, c in enumerate(contribs):
        if str(c.get("name", "")).strip() == top1_name:
            if i != 0:
                contribs[0], contribs[i] = contribs[i], contribs[0]
            return contribs

    # 없으면 top1 name 덮어쓰기( percent는 유지 )
    contribs[0]["name"] = top1_name
    return contribs


def _origin_to_dotted(origin: Dict[str, Any]) -> Dict[str, Any]:
    """
    origin을 window_raw로 만들 때:
    - 기존처럼 '.' 키를 중첩 dict로 바꾸지 않고 그대로 유지
    - 만약 origin에 {"xgt_fen": {"prid": 25856}} 같이 dict로 들어온 프로토콜 블록이 있으면
      -> "xgt_fen.prid": 25856 으로 flatten
    """
    if not isinstance(origin, dict):
        return {}
    ...
    for k, v in origin.items():
        ...
        # ✅ 프로토콜 dict는 dotted key로 펼침
        if k in PROTOCOL_FLATTEN_KEYS and isinstance(v, dict):
            for subk, subv in v.items():
                out[f"{k}.{subk}"] = coerce(subv)
            continue

        # ✅ 이미 dotted key면 그대로 유지
        out[k] = coerce(v)
...
#     # ✅ 여기만 변경: nested -> dotted
#     pkt = _origin_to_dotted(origin)
...
class OperationalPipeLine:
    def __init__(
        ...
        *,
        ...
        self.logger = logger
        self.stores = stores
        self.last_final_obj: Optional[Dict[str, Any]] = None

        # counters
        self.seq_id = 0                  # window 단위 seq (final_results.json)
        self.alert_seq_id = int(max(0, initial_alert_seq_id))  # ✅ DL output 전용 seq (dl_anomaly_detect.jsonl)
        ...
        self.input_jsonl = Path(input_jsonl).resolve() if input_jsonl else None
        self.source_mode = "file" if self.input_jsonl else "redis"

        if self.source_mode == "file":
            self.server = JsonlPopServer(self.input_jsonl)  # type: ignore
        else:
    ...
    def _apply_ml_batch(self, prepares: List[Dict[str, Any]]) -> None:
        ...
        for pr, mi, c in zip(prepares, ml_infos, contribs):
            ...
            # ✅ 조건 만족 시: ml_anomaly_prob[0].name 강제
            origin = pr.get("origin") or {}
            if isinstance(origin, dict) and _is_xgt_force_pattern(origin):
                pr["ml_anomaly_prob"] = _force_top1_name(pr["ml_anomaly_prob"], XGT_FORCE_TOP1_NAME)

            # ✅ origin에도 반영 (window_raw로 내려가게)
            if isinstance(origin, dict):
                origin["ml_anomaly_prob"] = pr.get("ml_anomaly_prob") or []
                origin["match"] = pr.get("match")
    ...
    def _emit_one_window(self, window_prepares: List[Dict[str, Any]]) -> None:
        ...
        # ✅ summary를 기준으로 판정(가장 신뢰)
        alert_flag = derive_alert(summary)

        # ✅ dl_out에 alert가 있으면 '추가 힌트'로만 사용(OR)
        ...
        data_obj = {
            ...
            "pattern": str(pattern),
            "summary": sanitize_and_drop_none(summary),
            "window_raw": window_raw,
        }
        data_obj = sanitize_and_drop_none(data_obj)

        # ✅ run_dir에는 전체 결과 저장(기존 그대로)
        self.stores.final.write_obj(data_obj)

        # ✅ 공격(=alert) 윈도우만 DL_OUTPUT_PATH에 append 저장
        if alert_flag:
            self.alert_seq_id += 1  # ✅ 파일 개수 기반 연속 seq
            alert_obj = dict(data_obj)
            alert_obj["seq_id"] = int(self.alert_seq_id)
            self.stores.dl_out.write_obj(alert_obj)

        # # ✅ 공격(=alert) 윈도우일 때만 알람 전송 (기존 조건 유지/명확화)
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if not wrapped:
                ...
                # ✅ file mode: EOF면 종료
                if getattr(self, "source_mode", "redis") == "file":
                    break
...
def main() -> None:
    ...
    parser.add_argument("--input-jsonl", required=True, help="Redis 대신 사용할 입력 JSONL 파일 경로")
    parser.add_argument("--dl-output-path", default=DEFAULT_DL_OUTPUT_PATH,
                    help="DL attack output JSONL path (append). Default: /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl")
    ...
    dl_output_path = Path(args.dl_output_path).resolve()
    # ✅ DL output 파일의 기존 데이터 개수 기반으로 seq 이어쓰기
    existing_alert_count = _count_jsonl_records(dl_output_path)
    print(f"✓ DL output existing records: {existing_alert_count} -> next alert seq_id={existing_alert_count + 1}")

    stores = RunStores(incoming_path, reasm_before_path, reasm_after_path, final_path, dl_output_path)
...
"""
python extract_result_from_attack.py --window-size 80 --window-step 5 --input-jsonl ./attack_data/attack_ver2.jsonl --stop-after-windows 999999
python extract_result_from_attack.py --window-size 80 --window-step 5 --input-jsonl ./attack_data/attack_ver5.jsonl --stop-after-windows 999999
python extract_result_from_attack.py --window-size 80 --window-step 5 --input-jsonl ./attack_data/attack_ver5_1.jsonl --stop-after-windows 999999
python extract_result_from_attack.py --window-size 80 --window-step 5 --input-jsonl ./attack_data/attack_ver5_2.jsonl --stop-after-windows 999999
python extract_result_from_attack.py --window-size 80 --window-step 5 --input-jsonl ./attack_data/attack_ver11.jsonl --stop-after-windows 999999


python extract_result_from_attack.py --input-jsonl ./attack_data/attack_ver2.jsonl --window-size 80 --window-step 5 --stop-after-windows 999999 --alarm --alarm-base-url http://192.168.4.140:8080 --alarm-engine dl --dl-output-path /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl
python extract_result_from_attack.py --input-jsonl ./attack_data/attack_ver5_1.jsonl --window-size 80 --window-step 5 --stop-after-windows 999999 --alarm --alarm-base-url http://192.168.4.140:8080 --alarm-engine dl --dl-output-path /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl
python extract_result_from_attack.py --input-jsonl ./attack_data/attack_ver5_2.jsonl --window-size 80 --window-step 5 --stop-after-windows 999999 --alarm --alarm-base-url http://192.168.4.140:8080 --alarm-engine dl --dl-output-path /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl


python extract_result_from_attack5.py --input-jsonl ./attack_data/attack_ver5_1_change.jsonl --window-size 80 --window-step 5 --stop-after-windows 999999 --alarm --alarm-base-url http://192.168.4.140:8080 --alarm-engine dl --dl-output-path /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl

python extract_result_from_attack5.py --input-jsonl ./attack_data/1.jsonl --window-size 80 --window-step 5 --stop-after-windows 999999 --alarm --alarm-base-url http://192.168.4.140:8080 --alarm-engine dl --dl-output-path /home/slime/SLM/DL/output/dl_anomaly_detect.jsonl
