...
"""
train_bayesian_lstm_ae_windows_keras.py

Keras/TensorFlow + TensorFlow Probability 버전의 Bayesian LSTM Autoencoder 학습 스크립트.

- Encoder LSTM → DenseFlipout(latent)
- Decoder LSTM → DenseFlipout(output)
- Flipout 층의 KL divergence가 loss에 자동으로 더해져 Bayesian 정규화
- train reconstruction error 계산 시 MC 샘플링 (--mc-samples) 사용 가능
"""
...
# -------------------------------------------------------
# MC 샘플링 기반 재구성 오차 계산 (Bayesian)
# -------------------------------------------------------
def mc_reconstruction_errors(
    model,
    X: np.ndarray,
    pad_value: float,
    batch_size: int,
    ...
    """
    Bayesian 모델에서 여러 번 forward (MC 샘플링) → 평균 예측으로 재구성 오차 계산.

    model     : Keras 모델 (DenseFlipout 포함)
    X         : (N, T, D)
    pad_value : 패딩 값
    mc_samples: forward 샘플링 횟수
    """
    preds = []
    for s in range(mc_samples):
        pred = model.predict(X, batch_size=batch_size, verbose=0)
        preds.append(pred)
    preds = np.stack(preds, axis=0)     # (S, N, T, D)
    mean_pred = preds.mean(axis=0)      # (N, T, D) - predictive mean

    errors = compute_window_errors(X, mean_pred, pad_value)
    return errors
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Bayesian 재구성 오차 계산 시 MC 샘플 수 (default: 5)",
    )
    ...
    # TensorFlow / Keras / TFP import
    import tensorflow as tf
    import tensorflow_probability as tfp
    import tf_keras

    layers = tf_keras.layers
    models = tf_keras.models
    ...
    print(f"[INFO] TensorFlow Probability version: {tfp.__version__}")
    ...
    # 3) Bayesian LSTM Autoencoder 모델 정의 (Keras + TFP)
    input_dim = D
    ...
    # KL weight: 데이터 개수에 맞게 스케일링 (간단히 1/N_train)
    kl_weight = 1.0 / float(X_train.shape[0])

    def kl_divergence_fn(q, p, _):
        return tfd.kl_divergence(q, p) * kl_weight
    ...
    # [BAYESIAN] latent Dense → DenseFlipout
    latent = tfpl.DenseFlipout(
        latent_dim,
        activation="relu",
    ...
    # [BAYESIAN] 출력층: DenseFlipout 을 시퀀스 전체에 바로 적용
    decoder_dense_flipout = tfpl.DenseFlipout(
        input_dim,
        activation=None,
        kernel_divergence_fn=kl_divergence_fn,
        name="decoder_output_flipout",
    )
    outputs = decoder_dense_flipout(decoder_output)         # (B, T, D)

    model = models.Model(inputs=encoder_inputs, outputs=outputs, name="bayesian_lstm_autoencoder")
    model.summary()

    # 4) 손실 함수 (pad_value 마스킹) + Flipout KL은 model.losses로 자동 추가
    ...
    print("[INFO] Bayesian Keras model.fit() 시작")
    ...
    # 6) train set reconstruction error 기반 threshold 계산 (MC 샘플링 사용)
    print("[INFO] train set Bayesian reconstruction error 계산 (MC 샘플링)...")
    errors_train = mc_reconstruction_errors(
        model,
        X_train,
        pad_value,
        batch_size=args.batch_size,
    ...
    # 7) 모델/설정/로그 저장
    # HDF5(.h5) 저장은 tfp DenseFlipout에서 pickle 문제를 일으킬 수 있어서
    # Keras native 포맷(.keras)로 저장 + 실패 시 weights만 저장
    model_path = output_dir / "model_bayesian.keras"
    try:
        model.save(model_path)  # Keras 포맷 (SavedModel 기반)
        print(f"[INFO] Bayesian 모델 저장 → {model_path}")
        model_weights_path = None
    except Exception as e:
        print(f"[WARN] model.save() 실패 (원인: {e})")
        model_weights_path = output_dir / "model_bayesian_weights.h5"
        model.save_weights(model_weights_path)
        print(f"[INFO] 모델 가중치만 저장 → {model_weights_path}")
    ...
    config = {
        ...
        "framework": "tf_keras + tfp",
        "seed": args.seed,
        "exclude_features": merged_exclude,
        "mc_samples": int(args.mc_samples),
        "bayesian": True,
        "model_path": str(model_path),
        "model_weights_path": str(model_weights_path) if model_weights_path is not None else None,
    }
    ...
    try:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Bayesian LSTM-AE Training / Validation Loss")
        plt.legend()
        plt.grid(True)
